...
export const getAllUsers = async () => {
    const result = await db.query('SELECT id, first_name, last_name, email, gender, role, created_at FROM users');
    return result.rows; 
};

export const getUserById = async (id) => {
    const result = await db.query('SELECT id, first_name, last_name, email, gender, role, created_at FROM users WHERE id = $1', [id]);
